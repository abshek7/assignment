...
const BookList = () => {
  ...
  const handleUpdate = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/books/${editBookId}`, {
        method: "PUT",
  ...
  const handleDelete = async (bookId) => {
    ...
    try {
      ...
      // If the last book on a page is deleted, go back a page if possible
                ...
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-gray-600">
                      Serial No: {(currentPage - 1) * booksPerPage + index + 1}
                    </p>
                  </div>
                  <div className="flex gap-2">
                    <Button variant="default" onClick={() => handleEdit(book)}>
                      Edit
                    </Button>
